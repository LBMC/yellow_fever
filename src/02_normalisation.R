...
table(sce$female_invivo)

norm_counts[["female_invivo_D90"]] <- SCnorm::SCnorm(
  Data = assays(sce)$counts_raw[rownames(norm_counts[["male_invivo"]]), sce$female_invivo & sce$day %in% "D90"],
  Conditions = rep(c(1), ncol(sce[, sce$female_invivo & sce$day %in% "D90"])),
  NCores = 1
)

save(norm_counts, file = "results/sce_norm_list.Rdata")

norm_counts[["female_invivo_D15"]] <- SCnorm::SCnorm(
  Data = assays(sce)$counts_raw[, sce$female_invivo & sce$day %in% "D15"],
  Conditions = rep(c(1), ncol(sce[, sce$female_invivo & sce$day %in% "D15"])),
...
norm_counts[["YVF2003_D1401"]] <- SCnorm::SCnorm(
  Data = assays(sce[, sce$YVF2003_D1401])$counts_raw,
  Conditions = rep(c(1), ncol(sce[, sce$YVF2003_D1401])),
  NCores = 1
)
save(norm_counts, file = "results/sce_norm_list.Rdata")
...
load(file = "results/sce_norm_list.Rdata", verbose = T)
norm_counts <- norm_counts[-2]
tmp <- assay(sce, "counts_raw") %>% Matrix::Matrix(sparse = T)
for (experiment in names(norm_counts)) {
  print(experiment)
  if (experiment %in% names(norm_counts)){
    colData(sce)[[experiment]] %>% 
      table() %>% 
      print()
    norm_counts[[experiment]] %>%
      assay("normcounts") %>%
      dim() %>%
      print()
    tmp[, colData(sce)[[experiment]]] <- norm_counts[[experiment]] %>%
      assay("normcounts")
  }
}
assay(sce, "normcounts") <- tmp
rm(experiment, norm_counts, tmp)


save(sce, file = "results/sce_scnorm.Rdata")
load(file = "results/sce_scnorm.Rdata", verbose = T)

rowData(sce)$gene_mean_norm <- rowMeans(assays(sce)$normcounts[, colData(sce)$to_QC])
rowData(sce)$gene_var_norm <- apply(assays(sce)$normcounts[, colData(sce)$to_QC], 1, var)
rowData(sce)$detection_rate_norm <- rowMeans(assays(sce)$normcounts[, colData(sce)$to_QC] > 0)
rowData(sce)$detected_norm <- rowSums(assays(sce)$normcounts[, colData(sce)$to_QC]) > 0
...
rowData(sce) %>% 
  ...
  geom_point(alpha = 0.3) +
  geom_density_2d(size = 0.3) +
  geom_point(aes(x = gene_mean_norm, y = detection_rate_norm), color = "green", alpha = 0.3) +
  geom_density_2d(size = 0.3) +
  geom_line(data = poisson_model,
            aes(x = mean, y = detection_rate), color = "red") +
  scale_x_log10() +
...
rowData(sce) %>% 
  ...
  geom_point(alpha = 0.3) +
  geom_density_2d(size = 0.3) +
  geom_point(aes(x = gene_mean_norm, y = gene_var_norm), color = "green", alpha = 0.3) +
  geom_abline(intercept = 0,
              slope = 1,
              color = 'red') +
  theme_bw() +
